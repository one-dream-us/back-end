...
//    }
    ...
    @Operation(summary = "뉴스 용어 스크랩하기", description = "특정 용어를 스크랩에 추가합니다. 'dictionaryId'로 용어를 선택해 스크랩할 수 있습니다.")
    @PostMapping("/dictionaries/{dictionaryId}")
    public ResponseEntity<String> scrapNewsTerm(
            @PathVariable("dictionaryId") Long dictionaryId,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        Users user = userDetails.getUser();
        historyService.scrapDictionary(dictionaryId, user);
        return ResponseEntity.ok("용어가 스크랩되었습니다.");
    }
    ...
    public ResponseEntity<DictionaryScrapResponse> getDictionaryScrapped(
        @AuthenticationPrincipal CustomUserDetails userDetails) {
        Users user = userDetails.getUser();
        DictionaryScrapResponse dictionaryScrapDtos = historyService.getDictionaryScrapped(user);
        return ResponseEntity.ok(dictionaryScrapDtos);
    }

    @Operation(summary = "스크랩한 용어 조회", description = "내가 스크랩한 모든 용어 목록을 확인할 수 있습니다.")
    @GetMapping("/news/dictionaries")
    public ResponseEntity<NewsDictionaryScrapResponse> getNewsDictionaryScrapped(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        Users user = userDetails.getUser();
        NewsDictionaryScrapResponse newsDictionaryScrapDtos = historyService.getNewsDictionaryScrapped(user);
    ...
    public ResponseEntity<String> deleteDictionaryScrapped(
        ...
        @AuthenticationPrincipal CustomUserDetails userDetails){
        Users user = userDetails.getUser();
        historyService.deleteDictionaryScrapped(dictionaryScrapId, user);
...
//    }
    ...
    public ResponseEntity<DictionaryScrapCntDto> getDictionaryScrapCnt(
        @AuthenticationPrincipal CustomUserDetails userDetails) {
        Users user = userDetails.getUser();
        DictionaryScrapCntDto dictionaryScrapCntDto = historyService.getDictionaryScrapCnt(user);
